...
use App\Http\Requests\Backend\FormLevelRequest;
use App\Models\Blog;
...
//        abort_if(Gate::denies('blogs index'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        if (request()->ajax()) {
            ...
            $start = request('start');
            $count = Level::count();
            $data = Level::latest()->offset($start)->limit($limit)->get();
...
//        abort_if(Gate::denies('blogs create'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        return view('backend.level.form', compact('edit'));
    }

    /**
     * @param FormLevelRequest $request
